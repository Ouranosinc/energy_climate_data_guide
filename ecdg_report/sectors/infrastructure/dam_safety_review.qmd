...
:::{#id1 .callout collapse="true"}
## Hydrological models
Hydrological models are important to 
@Fournier2020 create a report to value hydropower assets under climate change. This report also describe how to integrate climate change data inside the energy modelling chain and more precisely in an hydrologic model. Below are some of the important recommendations you can find in the guide:

* Climate simulations consideration:
    * Baseline: 30 years is recommended for the baseline to represent natural variability and avoid having predominant a dry or wet events. The baseline must represent current conditions. If [bias-correction](../../datasets/climate_projection_data/Climate_Projection_Datasets.ipynb) is applied to climate projections, the reference product must be the same used to calibrate the hyrdological models.
    * Consistency is important, variables must come from the same product (such as temperature and precipitation). 
    * Evaluate the perfomrance of various weather datasets in the hydrology model (weather stations, gridded observations and renalaysis). 
* Hydrologic modelling considerations:
    * Hydrologic baseline must be consistent with climatic baseline and representative of current conditions. 
    * Model need to be calibrated with the same reference used for [bias-correction](../../datasets/climate_projection_data/Climate_Projection_Datasets.ipynb).
    ...
    * Averaging multiple model simulations to run the hydrological can improve performance. 
    * Sensitivity analysis can be performed to adresse if sensitivty comes from the models parameters or climate simulations. 
    * It is ideal to used several hydrologic models and post-processing methods. If not, appropriate hydrological model selection (and climate simulations). 
